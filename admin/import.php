                ...
                if ($col_map['ref_id'] === null) {
                    ...
                    while (($row = fgetcsv($handle)) !== false) {
                        ...
                        $rows[] = [
                            ...
                            'body_type'        => gt_normalize_body_type($row[$col_map['body_type'] ?? -1] ?? ''),
                            'fuel_type'        => gt_normalize_fuel_type($row[$col_map['fuel_type'] ?? -1] ?? 'Petrol'),
                            'transmission'     => gt_normalize_transmission($row[$col_map['transmission'] ?? -1] ?? 'Automatic'),
